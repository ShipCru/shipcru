...
describe('buildIndustryWhere', () => {
  it('returns empty object for mode=all', () => {
    expect(buildIndustryWhere({ industryMode: 'all', industries: [] })).toEqual({})
  })

  it('returns in-clause for mode=include', () => {
    expect(buildIndustryWhere({ industryMode: 'include', industries: [1, 2] })).toEqual({
      id: { in: [1, 2] },
    })
  })

  it('returns not_in-clause for mode=exclude', () => {
    expect(buildIndustryWhere({ industryMode: 'exclude', industries: [3] })).toEqual({
      id: { not_in: [3] },
    })
  })
  ...
  it('handles populated objects', () => {
    ...
    expect(buildIndustryWhere({ industryMode: 'include', industries })).toEqual({
      id: { in: [5, 6] },
    })
  })
