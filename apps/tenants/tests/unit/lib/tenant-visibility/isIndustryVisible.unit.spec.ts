...
describe('isIndustryVisible', () => {
  it('returns true when mode is all', () => {
    expect(isIndustryVisible({ mode: 'all', industries: [] }, 1)).toBe(true)
  })

  it('returns true when mode is null (defaults to all)', () => {
    expect(isIndustryVisible({ mode: null, industries: [] }, 1)).toBe(true)
  })

  it('returns true when mode is include and industry is in list', () => {
    expect(isIndustryVisible({ mode: 'include', industries: [1, 2] }, 1)).toBe(true)
  })

  it('returns false when mode is include and industry is not in list', () => {
    expect(isIndustryVisible({ mode: 'include', industries: [2, 3] }, 1)).toBe(false)
  })

  it('returns false when mode is include and list is empty', () => {
    expect(isIndustryVisible({ mode: 'include', industries: [] }, 1)).toBe(false)
  })

  it('returns true when mode is exclude and industry is not in list', () => {
    expect(isIndustryVisible({ mode: 'exclude', industries: [2, 3] }, 1)).toBe(true)
  })

  it('returns false when mode is exclude and industry is in list', () => {
    expect(isIndustryVisible({ mode: 'exclude', industries: [1, 2] }, 1)).toBe(false)
  })

  it('handles populated objects with { id }', () => {
    const industries = [{ id: 5 }, { id: 6 }] as Array<{ id: number } | number>
    expect(isIndustryVisible({ mode: 'include', industries }, 5)).toBe(true)
    expect(isIndustryVisible({ mode: 'include', industries }, 7)).toBe(false)
  })
})
