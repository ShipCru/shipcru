...
/**
 * Raw query — loads the default industry template global.
 */
export async function getDefaultTemplate(payload: Payload) {
  return payload.findGlobal({ slug: 'default-industry-template' })
}
...
export const getCachedDefaultTemplate = unstable_cache(
  ...
  { tags: [generateCacheTag({ type: 'global', slug: 'default-industry-template' })] },
)
